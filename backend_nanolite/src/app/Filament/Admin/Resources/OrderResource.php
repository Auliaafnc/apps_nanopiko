            ...
            Repeater::make('products')->label('Detail Produk')->reactive()->live()
                    ...
                    Select::make('produk_id')
                        ...
                        ->afterStateUpdated(function ($state, callable $set, callable $get) {
                            ...
                            if ($p) {
                                ...
                                // poin
                                $units = (int) floor($after / 1_000_000);
                                $set('../../reward_point', $units);
                                $set('../../jumlah_program', $get('../../customer_program_id') ? $units : 0);
                            } else {
                                $set('price', 0);
                                $set('subtotal', 0);
                                ...
                                $units = (int) floor(((int) $totalHarga) / 1_000_000);
                                $set('../../reward_point', $units);
                                $set('../../jumlah_program', $get('../../customer_program_id') ? $units : 0);
                            }
                        })
                ...
                TextInput::make('reward_point')
                    ...
                    ->afterStateHydrated(function (callable $set, callable $get) {
                        ...
                        $set('reward_point', (int) floor($afterTax / 1_000_000));
                    })->reactive(),
                ...
                TextInput::make('total_harga_after_tax')
                    ...
                    ->afterStateHydrated(function (callable $set, callable $get) {
                        ...
                        if (! ($get('diskons_enabled') ?? false)) {
                            $set('total_harga_after_tax', (int) round($total));
                            $units = (int) floor($total / 1_000_000);
                            $set('reward_point', $units);
                            $set('jumlah_program', $get('customer_program_id') ? $units : 0);
                            return;
                        }
                    ...
                    ->afterStateUpdated(function ($state, callable $set, callable $get) {
                        $units = (int) floor(((float) ($state ?? 0)) / 1_000_000);
                        $set('reward_point', $units);
                        $set('jumlah_program', $get('customer_program_id') ? $units : 0);
                    }),
                ...
                TextInput::make('jumlah_program')
                    ...
                    ->afterStateHydrated(function (callable $set, callable $get) {
                        $afterTax = (float) ($get('total_harga_after_tax') ?? 0);
                        $units = (int) floor($afterTax / 1_000_000);
                        $set('jumlah_program', $get('customer_program_id') ? $units : 0);
                    })->reactive(),
